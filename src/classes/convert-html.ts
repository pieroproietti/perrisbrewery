...
export default function convertHtml() {
  if (fs.existsSync('pb.yaml')) {
    ...
    let destHtml = pbPackage.destDir + '/usr/lib/' + pbPackage.name + '/manpages/doc/man/' + pbPackage.name + '.1.html'
    if (pbPackage.name === 'eggs') {
      destHtml = pbPackage.destDir + '/usr/lib/penguins-eggs/manpages/doc/man/' + pbPackage.name + '.1.html'
    }
